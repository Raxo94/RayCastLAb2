...
void buildScene()
{
	if (shapes.size() == 0)
	{
		...
		shapes.push_back(dynamic_cast<Shape*>(new MyPlane(Vec(0, 0, -1), -1000, Color(0, 40, 60))));
		...
		shapes.push_back(dynamic_cast<Shape*>(new MyOBB(Vec(g_ToScreen->mScreenWidth/2, g_ToScreen->mScreenHeight/2, 100), b1, b2, b3, 50, 50, 50, { 0,255,0 })));
		...
		b2.y = tempY;
		shapes.push_back(dynamic_cast<Shape*>(new MyOBB(Vec(200,600,400), b1,b2,b3, 100, 100, 100, {255,0,0})));
	}
}
